...
object ActDemo {
	...
	val timerEvent = Anim(ClockTick(1, 10), (t: Double) => { g.fill(Color.SeaGreen).translate(t * 20, 100) } )
	val clickEvent = Anim(MouseClick, (xy: (Int, Int)) => { g.fill(Color.Violet).center.translate(xy._1, xy._2) } )
