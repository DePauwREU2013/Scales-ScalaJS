...
object ActDemo {
	...
	val timerEvent = Reactor(ClockTick(1, 10), (t: Double) => { g.fill(Color.SeaGreen).translate(t * 20, 100) } )
	val clickEvent = Reactor(MouseClick, (xy: (Int, Int)) => { g.fill(Color.Violet).center.translate(xy._1, xy._2) } )



	val actor = (Act(g) on Act(note))
	val actor2 = (Act(lessObnoxiousNote) seq Act(g))
	val actor3 = (actor2 on Act(timerEvent)) seq Act(note) //less obnoxious note and then graphic while moving and then shrill note
