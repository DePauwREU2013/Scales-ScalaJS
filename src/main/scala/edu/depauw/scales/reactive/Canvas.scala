...
object CanvasHandler {
  ...
  def setFillColor(index: Int, c: Color): Unit = {
    fillStyles() = fillStyles().patch(index, List(c.toString), 1)
  }

  def setStrokeColor(index: Int, c: Color): Unit = {
    strokeStyles() = strokeStyles().patch(index, List(c.toString), 1)
  }
  ...
  def getId(id: String): Int = {
    if(ids().isDefinedAt(id) == false) return -1
    else ids()(id)
  }

  def setAll(index: Int, fillStyle: Color, strokeStyle: Color, width: Int, id: String): Unit = {
    fillStyles() = fillStyles().patch(index, List(fillStyle.toString), 1)
    strokeStyles() = strokeStyles().patch(index, List(strokeStyle.toString), 1)
    lineWidths() = lineWidths().patch(index, List(width), 1)
    setId(index, id)
  }
